...
const useFirebase = () => {
    ...
    const registerUser = (email, password, name, history) => {
        setIsLoading(true);
        createUserWithEmailAndPassword(auth, email, password, signOut)
            .then((userCredential) => {
                setAuthError('');
    ...
    useEffect(() =>{
        fetch(`https://immense-wildwood-34940.herokuapp.com/users/${user.email}`)
        .then(res => res.json())
    ...
    const saveUser = (email, displayName, method) => {
            ...
            fetch('https://immense-wildwood-34940.herokuapp.com/users', {
                method: method,
                headers: {
